...
def submit_donation():
    ...
    mask = (item_list["名稱"] == item) & (item_list["短宣隊"] == team)
    item_idx = item_list.index[mask].item()

    if not name or number == 0 or number > item_list.loc[item_idx, "剩餘數量"]:
...
item, team = re.split("（|）", item_name)[:2]
item_idx = item_list.index[(item_list["名稱"] == item) & (item_list["短宣隊"] == team)].item()

st.text(f"金額：${item_list.loc[item_idx, '單價']}/份")
st.text(f"剩餘：{item_list.loc[item_idx, '剩餘數量']}份")
...
number = st.number_input("數量", 0, item_list.loc[item_idx, "剩餘數量"], key="number")
total_money = item_list.loc[item_idx, "單價"] * number
